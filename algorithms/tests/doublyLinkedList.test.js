...
test(`insert test`, () => {
  ...
  expect(list2.insert(0, "Ozymandias")).toBe(true)
...
test(`remove`, () => {
  ...
  expect(list.get(1).val).toBe("traveller")
  expect(list.get(1).prev.val).toBe("met")
  let nothing = list.remove(list.length);
...
test(`reverse`, () => {
  ...
  list.reverse(); // singlyLinkedList;
  let length = list.length; // 4
  expect(length).toEqual(4)
  expect(list.head.val).toEqual(20) // 20
  expect(list.head.next.val).toEqual(15) // 15
  expect(list.head.next.next.val).toEqual(10) // 10
  expect(list.head.next.next.next.val).toEqual(5) // 5
})
